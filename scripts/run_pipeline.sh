...
ROWS="${ROWS:-1000000}"
INPUT="$ROOT_DIR/data/generated/phone_attribution.csv"
OUTPUT="$ROOT_DIR/artifacts/guangzhou-count.json"

"$PYTHON_BIN" "$ROOT_DIR/scripts/generate_data.py" \
  --rows "$ROWS" \
  --output "$INPUT"
...
"$SPARK_HOME/bin/spark-submit" \
  ...
  "$ROOT_DIR/src/spark_jobs/guangzhou_count.py" \
  --input "$INPUT" \
  --output "$OUTPUT"
